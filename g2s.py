...
# inputGroupDataFileName, indexColName, valueColName, globalAlpha, noFilterBool, outputDir = "/data1/bio/projects/tgrigoreva/25_ecoli_genes/coverages/pvals/25_ecoli_genes.groupdata", "reference_id", "id_mapped_relative_abundance", 0.05, False, "/data1/bio/projects/tgrigoreva/25_ecoli_genes/coverages/pvals/test/"

# input_df = tmp
#
# def mp_count_rejected_null_hypothesis(series):
#     output_dict = {"null_hypothesis_rejected_counter": len([i for i in series.values.tolist() if i == 'True'])}
#     return pd.Series(ComparingGroupParser.dict2pd_series(output_dict), name=series.name)
#
# queue = [input_df.loc[i, :] for i in input_df.index.tolist()]
# df = pd.concat(multi_core_queue(mp_count_rejected_null_hypothesis, queue), axis=1).transpose()
# output_df = pd.concat([input_df, df], axis=1).sort_values(by="null_hypothesis_rejected_counter", ascending=False)
# output_df.sort(columns=[])
#
# index_str = input_df.index.tolist()[0]
#
# series = input_df.loc[index_str, :]
...
# for group in groupsParsedList:
#     df = group.create_base_stats_df()
#     df.rename(columns={i: group.name + "_" + i for i in list(df)}, inplace=True)
#     output_list.append(df)
# pd.concat(output_list, axis=1)
...
# comparison = groupComparisonsList[0]
#
# group_1 = comparison.group_pair_list[0]
# group_2 = comparison.group_pair_list[-1]
# df_1 = group_1.create_base_stats_df()
# df_2 = group_2.create_base_stats_df()
#
# prevalent_function_name = "sum"
# df = pd.concat([i.loc[:, [prevalent_function_name]].rename(columns={c: j for c in list(i)}) for (i, j) in [(o.create_base_stats_df(), o.name) for o in [group_1, group_2]]], axis=1)
#
# index_string = df.index.tolist()[0]
#
# input_dict = df.loc[index_string, :].to_dict()
# key_1, key_2 = input_dict
# if input_dict[key_1] > input_dict[key_2]:
#     output_dict = {"prevalent": key_1}
# elif input_dict[key_1] < input_dict[key_2]:
#     output_dict = {"prevalent": key_2}
# else:
#     output_dict = {"prevalent": np.nan}
# pd.Series(ComparingGroupParser.dict2pd_series(output_dict), name=index_string)

# output_list = []
# for group in groupsParsedList:
#     df = pd.concat(multi_core_queue(group.get_values_series, indexList), axis=1).transpose()
#     df.rename(columns={i: group.name + "_" + i for i in list(df)}, inplace=True)
#     output_list.append(df)
#
# pd.concat(output_list, axis=1)

# df = pd.concat([pd.concat(multi_core_queue(i.get_values_series, indexList), axis=1).transpose() for i in groupsParsedList], axis=1)

# index_string = indexList[0]
# df = groupsParsedList[0].pivot_df
#
# try:
#     input_dict = df.loc[index_string].to_dict()
#     output_dict = {}
...
#             output_dict.update({k: 0})
# except KeyError:
#     output_dict = {i: 0 for i in list(df)}
#
# pd.Series(ComparingGroupParser.dict2pd_series(output_dict), name=index_string)

# mc_test = MultipleComparator()
# mc_test.multi_pvals_dfs_dict
# list(mc_test.multi_pvals_dfs_dict['t-test'])
# len(mc_test.multi_pvals_functions_list)

# len(groupComparisonsList)
# single_pvals_dfs_dict = {i: [] for i in GroupComparator.create_functions_dict()}
# for single_pvals_method in single_pvals_dfs_dict:
#     for comparison in groupComparisonsList:
#         df = comparison.dfs_dict[single_pvals_method].loc[:, ["p-value"]]
#         df.rename(columns={"p-value": "_vs_".join(comparison.group_names_list)}, inplace=True)
#         single_pvals_dfs_dict[single_pvals_method].append(df)
#
# for single_pvals_method in single_pvals_dfs_dict:
#     # Replacing 'NA' with 1
#     df = pd.concat(single_pvals_dfs_dict[single_pvals_method], axis=1).fillna(1)

# index_string = df.index.tolist()[0]
# input_dict = {"index_string": index_string, "pvals_list": df.loc[index_string].values.tolist(), "colnames_list": list(df)}
#
# output_dict = {}
# method = "bonferroni"
# rejects_list, pvals_corrected_list, alphac_sidak, alphac_bonf = statsmodels.sandbox.stats.multicomp.multipletests(pvals=input_dict["pvals_list"], method=method, alpha=globalAlpha)
# output_dict.update({i + "_is_rejected_by_" + method: j for (i, j) in zip(input_dict["colnames_list"], [str(k) for k in rejects_list])})
# output_dict.update({i + "_corrected_for_" + method: j for (i, j) in zip(input_dict["colnames_list"], pvals_corrected_list)})
# output_dict.update({"raw_alpha": globalAlpha, "alpha_corrected_for_Sidak_method": alphac_sidak, "alpha_corrected_for_Bonferroni_method": alphac_bonf})
# pd.Series(ComparingGroupParser.dict2pd_series(output_dict), name=input_dict["index_string"])


# gc_test = GroupComparator(groupsPairs2DArray[0])
# gc_test_df = gc_test.group_comparison_df
# pd.concat(gc_test_df, axis=1).transpose()

# self_group_pair_list = groupsPairs2DArray[0]
# index_string = "NC_000913_3_1019013_1020053_Escherichia_coli_str_K_12_substr_MG1655_complete_genome_RETRIEVED_BY_ompA_WITH_NANE_ompA"
#
# group_1 = self_group_pair_list[0]
# group_2 = self_group_pair_list[-1]
# list_1 = group_1.get_values_list(index_string)
# list_2 = group_2.get_values_list(index_string)
# dict_1 = group_1.list2base_stats(list_1)
# dict_2 = group_2.list2base_stats(list_2)
# output_dict = {group_1.name + "_" + k: dict_1[k] for k in dict_1}
# output_dict.update({group_2.name + "_" + k: dict_2[k] for k in dict_2})
#
# functions_dict = {"t-test": scipy.stats.ttest_ind,
#                   "u-test": scipy.stats.mannwhitneyu,
#                   "wilcoxon": scipy.stats.ranksums,
#                   "h-test": scipy.stats.kruskal}
#
# for function_name in functions_dict:
#     if int(output_dict[group_1.name + "_" + "non-zero_values"]) == 0 and int(output_dict[group_2.name + "_" + "non-zero_values"]) == 0:
#         output_dict.update({function_name + "_statistic": np.nan, function_name + "_p-value": np.nan})
#     else:
#         output_dict.update({i: j for (i, j) in zip([function_name + "_" + k for k in ["statistic", "p-value"]], functions_dict[function_name](list_1, list_2))})
#
# pd.Series(ComparingGroupParser.dict2pd_series(output_dict), name=index_string)

# cgp_test = ComparingGroupParser(groupsRawList[0])
